...
# UI
# ----------------------------
st.title("Mortgage Spread: 30Y Fixed Mortgage – 10Y Treasury (with Recession Overlays)")
...
with st.sidebar:
    ...
    regime_threshold_wide = st.slider("“Wide” regime threshold (pp)", 0.5, 4.0, 2.0, 0.1)
    regime_threshold_tight = st.slider("“Tight” regime threshold (pp)", -2.0, 2.0, 0.5, 0.1)
...
c1.metric("Latest spread", format_pp(latest), help="Mortgage30Y − Treasury10Y")
c2.metric("As-of date", "—" if latest_date is None else latest_date.strftime("%Y-%m-%d"))
c3.metric("Percentile (history)", "—" if pd.isna(pct) else f"{pct:.0f}th")
c4.metric("Z-score (history)", "—" if pd.isna(z) else f"{z:.2f}")
